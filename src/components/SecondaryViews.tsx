import { CalendarDays, FileBarChart, Route } from "lucide-react";
...
export function StatisticsPage({ stats }: { stats: TripStats }) {
  return <><div className="page-intro"><h1>Statistiken</h1></div><section className="insights-grid"><article className="insight-card"><div className="settings-heading"><span><h2>Rollenverteilung</h2><p>{stats.total} Fahrten insgesamt</p></span></div><div className="bar-list"><div><span>Alleinfahrten <b>{stats.driverPercent} %</b></span><i><em style={{ width: `${stats.driverPercent}%` }} className="green" /></i></div><div><span>Beifahrer-Fahrten <b>{stats.passengerPercent} %</b></span><i><em style={{ width: `${stats.passengerPercent}%` }} className="purple" /></i></div></div></article><article className="insight-card cost-highlight"><small>Mitfahrkosten gesamt</small><strong>{formatCurrency(stats.totalCostCents)}</strong><p>Über alle gespeicherten Beifahrer-Fahrten</p></article></section></>;
}

export function CostsPage({ trips }: { trips: Trip[] }) {
  const passengerTrips = trips.filter((trip) => trip.role === "passenger");
  return <><div className="page-intro"><h1>Kosten</h1></div><section className="simple-list-card"><div className="card-heading"><h2>Mitfahrkosten</h2><strong>{formatCurrency(passengerTrips.reduce((sum, trip) => sum + trip.totalCostCents, 0))}</strong></div>{passengerTrips.length ? passengerTrips.map((trip) => <article className="cost-list-row" key={trip.id}><span><strong>{trip.startLocation} → {trip.destination}</strong><small>{formatDate(trip.date)} · {directionLabel(trip)}</small></span><b>{formatCurrency(trip.totalCostCents)}</b></article>) : <div className="empty-trips compact"><h3>Noch keine Mitfahrkosten</h3><p>Beifahrer-Fahrten erscheinen automatisch in dieser Übersicht.</p></div>}</section></>;
}
...
export function PlaceholderPage({ type }: { type: "calendar" | "routes" | "reports" }) {
  ...
  return <><div className="page-intro"><h1>{content.title}</h1></div><section className="placeholder-card"><Icon /><h2>{content.title} wird erweitert</h2><p>{content.text}</p><small>Deine bestehenden Fahrten bleiben davon unberührt und weiterhin vollständig lokal gespeichert.</small></section></>;
}
