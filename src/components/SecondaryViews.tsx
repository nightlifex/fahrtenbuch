import { BarChart3, CalendarDays, CarFront, Construction, FileBarChart, MapPin, Route, UsersRound, WalletCards } from "lucide-react";
...
export function TripsPage({ trips, settings, onSave, onDelete, onToast }: { trips: Trip[]; settings: AppSettings; onSave: (trip: Trip) => Promise<void>; onDelete: (id: string) => Promise<void>; onToast: (message: string) => void }) {
  ...
  return <><div className="page-intro"><span><p>Gesamte Historie</p><h1>Fahrten</h1></span></div>{editing && <TripForm settings={settings} editingTrip={editing} onSave={onSave} onCancelEdit={() => setEditing(undefined)} onSuccess={onToast} />}<RecentTrips trips={trips} onEdit={setEditing} onDelete={setDeleting} />{deleting && <Dialog title="Fahrt löschen?" danger onClose={() => setDeleting(undefined)}><p>Möchtest du die Fahrt von <strong>{deleting.startLocation}</strong> nach <strong>{deleting.destination}</strong> wirklich löschen?</p><div className="dialog-actions"><button className="button secondary" onClick={() => setDeleting(undefined)}>Abbrechen</button><button className="button danger" onClick={async () => { await onDelete(deleting.id); setDeleting(undefined); onToast("Fahrt wurde gelöscht."); }}>Fahrt löschen</button></div></Dialog>}</>;
}

export function StatisticsPage({ stats }: { stats: TripStats }) {
  return <><div className="page-intro"><span><p>Deine Fahrgewohnheiten</p><h1>Statistiken</h1></span></div><section className="insights-grid"><article className="insight-card"><div className="settings-heading"><span className="settings-icon blue"><BarChart3 /></span><span><h2>Rollenverteilung</h2><p>{stats.total} Fahrten insgesamt</p></span></div><div className="bar-list"><div><span><CarFront />Alleinfahrten <b>{stats.driverPercent} %</b></span><i><em style={{ width: `${stats.driverPercent}%` }} className="green" /></i></div><div><span><UsersRound />Beifahrer-Fahrten <b>{stats.passengerPercent} %</b></span><i><em style={{ width: `${stats.passengerPercent}%` }} className="purple" /></i></div></div></article><article className="insight-card cost-highlight"><span className="settings-icon orange"><WalletCards /></span><small>Mitfahrkosten gesamt</small><strong>{formatCurrency(stats.totalCostCents)}</strong><p>Über alle gespeicherten Beifahrer-Fahrten</p></article></section></>;
}

export function CostsPage({ trips }: { trips: Trip[] }) {
  const passengerTrips = trips.filter((trip) => trip.role === "passenger");
  return <><div className="page-intro"><span><p>Fahrgemeinschaften</p><h1>Kosten</h1></span></div><section className="simple-list-card"><div className="card-heading"><h2><WalletCards />Mitfahrkosten</h2><strong>{formatCurrency(passengerTrips.reduce((sum, trip) => sum + trip.totalCostCents, 0))}</strong></div>{passengerTrips.length ? passengerTrips.map((trip) => <article className="cost-list-row" key={trip.id}><span className="tone-purple"><UsersRound /></span><span><strong>{trip.startLocation} → {trip.destination}</strong><small>{formatDate(trip.date)} · {directionLabel(trip)}</small></span><b>{formatCurrency(trip.totalCostCents)}</b></article>) : <div className="empty-trips compact"><WalletCards /><h3>Noch keine Mitfahrkosten</h3><p>Beifahrer-Fahrten erscheinen automatisch in dieser Übersicht.</p></div>}</section></>;
}
